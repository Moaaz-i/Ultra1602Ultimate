...
Ultra1602Ultimate::Ultra1602Ultimate(uint8_t rs, uint8_t en, uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7, uint8_t bl_pin) {
...
void Ultra1602Ultimate::begin(uint8_t cols, uint8_t rows) {
    _cols = (cols > 20) ? 20 : cols; _rows = (rows > 4) ? 4 : rows;
    _curr_col = 0; _curr_row = 0;

    for (uint8_t r = 0; r < 4; r++) {
        for (uint8_t c = 0; c < 20; c++) _shadowBuffer[r][c] = 0x00;
    }
    ...
    if (_marqueeActive && _marqueeText != NULL) {
        if (currentTime - _marqueeLastTime >= _marqueeInterval) {
            ...
            for (uint8_t j = 0; j < _cols; j++) {
                ...
                if (idx >= 0 && idx < len) write(_marqueeText[idx]);
                else write(' ');
            }
    ...
    if (_blinkActive && _blinkText != NULL) {
        if (currentTime - _blinkLastTime >= _blinkInterval) {
            ...
            if (_blinkState) {
                ...
                for(uint8_t i=0; i<len; i++) write(' ');
            }
        }
    }
}

void Ultra1602Ultimate::startMarqueeAsync(const char* text, uint8_t row, uint16_t interval_ms) {
    _marqueeText = text;
    _marqueeRow = row;
...
void Ultra1602Ultimate::startBlinkingTextAsync(const char* text, uint8_t col, uint8_t row, uint16_t blink_interval_ms) {
    _blinkText = text;
    _blinkCol = col;
    _blinkRow = row;
...
void Ultra1602Ultimate::printAdvanced(const char* str, uint8_t col, uint8_t row, TextProps props) {
    ...
    if (props.align == 1) startCol = (len >= _cols) ? 0 : (_cols - len) / 2;
...
void Ultra1602Ultimate::drawPreciseProgressBar(uint8_t row, uint8_t percentage, uint8_t startCol, uint8_t endCol) {
    if (percentage > 100) percentage = 100;
    if (endCol >= _cols) endCol = _cols - 1;
    ...
    for (uint8_t i = 0; i < barWidth; i++) {
        ...
        if (i < fullBars) write(5);
        else if (i == fullBars && remainder > 0) write(0);
        else write(' ');
    }
}
...
void Ultra1602Ultimate::setBacklightBrightness(uint8_t brightness) {
    if (_bl_pin != 255) analogWrite(_bl_pin, brightness);
}

void Ultra1602Ultimate::pulseEnable() {
    digitalWrite(_en_pin, LOW); delayMicroseconds(1);
    digitalWrite(_en_pin, HIGH); delayMicroseconds(1);
    digitalWrite(_en_pin, LOW); delayMicroseconds(100);
}
